...
from app.config import DEFAULT_HISTORY_PERIOD
...
def _fetch_mubasher_history(symbol: str, period: str = "2y"):
    """
    جلب البيانات من الـ API المخفي الخاص بـ TradingView على موقع مباشر.
    بيجيب بيانات دقيقة جداً للسوق المصري بدون مشاكل حظر.
    """
    symbol = symbol.upper().strip()
    # مباشر بيتعرف على أسهم مصر بوضع لاحقة EGX: قبل الرمز
    mubasher_symbol = f"EGX:{symbol}"
    
    # حساب التواريخ (من - إلى) بالثواني (Unix Timestamp)
    end_time = int(time.time())
    
    # تحديد عدد السنين لعمل الباك-تيست (سنتين أو سنة)
    years = 2 if "2y" in period else 1
    start_time = end_time - (years * 365 * 24 * 60 * 60)
    
    # رابط الـ UDF API الخاص بمباشر
    url = "https://www.mubasher.info/api/1/chart/udf/history"
    params = {
        "symbol": mubasher_symbol,
        "resolution": "D",  # فريم يومي
        "from": start_time,
        "to": end_time
    }
    
    # بصمة متصفح عشان السيرفر يقبل الطلب
    headers = {
    ...
    # صيغة TradingView بترجع s='ok' لو لقى بيانات
    if data.get('s') != 'ok' or not data.get('t'):
        raise Exception(f"لا توجد بيانات متاحة لهذا الرمز على مباشر: {symbol}")
        
    # بناء الـ DataFrame بسرعة من الـ JSON
    df = pd.DataFrame({
        'Open': data['o'],
        'High': data['h'],
        'Low': data['l'],
...
def fetch_history(
    ...
    """
    الدالة الرئيسية اللي المنصة بتنادي عليها.
    """
